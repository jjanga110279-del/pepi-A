...
export default function Header() {
  ...
  const isDetailPage = ['/about', '/terms', '/guide', '/privacy', '/customer-service', '/best50', '/new-in'].includes(location.pathname);
        ...
        <div className="max-w-[1920px] mx-auto px-4 md:px-10">
          <div className="flex items-center justify-between relative h-12 md:h-14">
            ...
            <div className="flex items-center gap-4 md:gap-8 z-10">
              ...
              <button className="text-sm md:text-[16px] font-bold font-hei text-black/90 hover:text-[#dc2626] tracking-tight hidden sm:block transition-colors">로그인 / 회원가입</button>
          ...
          {/* Desktop Navigation for Listing Pages (Best 50, New In) */}
          {(isBest50Page || isNewInPage) && (
            <nav className={`hidden lg:flex justify-center items-center gap-14 mt-10 pb-6 transition-all duration-300 ${isScrolled ? 'scale-95' : ''}`}>
              ...
              {['신상 5%', '아우터', '상의', '하의', '원피스', '세트', '액세서리', 'SALE', '이벤트/기획전'].map((item) => {
                ...
                const isActive = isNewIn && isNewInPage;
        ...
        {isMobileMenuOpen && (
          <div className="fixed inset-0 z-[100] bg-white lg:hidden animate-fadeIn flex flex-col">
            ...
            <div className="p-6 pt-12 border-t border-black/5 flex flex-col gap-4 mt-auto">
              <button className="text-sm font-hei text-left hover:text-[#dc2626] transition-colors">로그인 / 회원가입</button>
