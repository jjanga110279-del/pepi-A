...
export default function Top() {
  ...
  const allAvailableImages = useMemo(() => {
    const images = [
      // 직접 올리신 로컬 이미지 (상의 위주 선별 가능)
      IMAGES.best1, IMAGES.best3, IMAGES.best7, IMAGES.best8, IMAGES.best11, 
      IMAGES.best14, IMAGES.best15, IMAGES.best17, IMAGES.best18,
      IMAGES.best2, IMAGES.best4, IMAGES.best5, IMAGES.best6, IMAGES.best9, 
      IMAGES.best10, IMAGES.best12, IMAGES.best13, IMAGES.best16,
      
      // 프로젝트 내 기존 유효 상의/범용 이미지
      IMAGES.productLinenShirt, IMAGES.productBlouse, IMAGES.newScarf,
      IMAGES.bestItem1, IMAGES.recItem2, IMAGES.hero1, IMAGES.hero3,
      
      // 검증된 상의 관련 고화질 Unsplash 이미지
  ...
  const generateTopProducts = () => {
    const products = [];
    const subData = {
      '나시': ['골지 슬림핏 민소매', '실크 스트랩 탑', '코튼 베이직 나시', '린넨 레이어드 나시'],
      '티': ['컴포트 베이직 티셔츠', '오버핏 레터링 티', '스트라이프 보트넥 티', '유넥 슬림핏 티'],
      '니트': ['소프트 라운드 넥 니트', '스퀘어 넥 반팔 니트', '캐시미어 풀오버', '배색 라인 골지 니트'],
      '블라우스': ['실크 버튼 블라우스', '시폰 플라워 블라우스', '오버사이즈 린넨 셔츠', '셔링 퍼프 블라우스']
    };
    ...
    Object.keys(subData).forEach(subName => {
      ...
      for (let i = 1; i <= 30; i++) {
        ...
        const basePrice = 32000 + (Math.floor(Math.random() * 20) * 5000);
        ...
        products.push({
          ...
          isNew: isNewItem,
          isBest: i % 7 === 0,
          reviews: Math.floor(Math.random() * 180 + 10),
  ...
  const allTopProducts = useMemo(() => {
    const realTopItems = ALL_PRODUCTS.filter(p => p.category === 'top');
    return [...realTopItems, ...generateTopProducts()];
  }, [allAvailableImages]);
