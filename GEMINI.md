...
- **Header Design (Detail/Listing Pages)**: 상세 페이지 및 리스팅 페이지(신상, 베스트 50 등)의 헤더는 현재의 확대된 스타일을 유지합니다.
    ...
    - **No Black Boxes**: 앞으로 모든 배지(Badge)와 버튼에 **검정색 배경 박스(`bg-black`) 사용을 금지**합니다. 배지는 흰색 배경에 테두리가 있는 스타일이나 브랜드 포인트 컬러를 사용하며, 보조 버튼은 홈 화면의 '더보기'와 같은 밝은 회색 스타일을 우선적으로 사용합니다.
